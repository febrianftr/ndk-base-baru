...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    $pk_study_move = defaultValue($row['pk_study']);
    $pk_patient_move = defaultValue($row['pk_patient']);
    ...
    $name_attr = $is_study_series == 'study' ? "name='save_move_study'" : "name='save_move_series'";
    $button = "<input type='hidden' name='move' value='$pk_study_move'>
        <input type='hidden' name='current' value='$pk_child'>
        <input type='hidden' name='study_iuid_current' value='$study_iuid_parent'>
        <input type='hidden' name='study_iuid_move' value='$study_iuid'>
        <button type='submit' $name_attr style='width: 100%;margin: 0;padding: 5px 5px; font-size:12px;background-color: #fff;color: #3443eb;border: 2px solid #3443eb;border-radius: 7px;' class='ahref-edit btn btn-sm btn-delete-study' onclick=\"return confirm('Are you sure move?');\">Move $is_study_series</button>";
