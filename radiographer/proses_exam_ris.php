...
@$uid = $_POST['study_iuid'];
@$kv = $_POST['kv'];
@$mas = $_POST['mas'];
@$filmsize8 = $_POST['filmsize8'];
@$filmsize10 = $_POST['filmsize10'];
@$filmreject8 = $_POST['filmreject8'];
@$filmreject10 = $_POST['filmreject10'];
@$rephoto = $_POST['rephoto'];
@$radiographer_name = addslashes($_POST['radiographer_name']);
...
if (mysqli_affected_rows($conn) > 0) {
	...
	echo "<script>alert('Succsess to $dokradid Worklist');</script>
    	<script>document.location.href='exam2.php';</script>";
	mysqli_close($conn);
} else {

	echo "<script>alert('Failed tos $dokradid process');</script>
    	<script>document.location.href='exam2.php';</script>";
	mysqli_close($conn);
}
