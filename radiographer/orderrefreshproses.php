...
$weight = $row['weight'];
$name_dep = str_replace("&", "dan", $row['name_dep']);
$xray_type_code = $row['xray_type_code'];
// if ($xray_type_code == "DX") {
//   $name1 = str_replace(' ','_', $name);
// } else {
// 	$name1 = $name;
// }
// if ($xray_type_code == "CR") {
//   $patientid = $patientid .' '. 'CR';
// }
$typename = $row['typename'];
$type = $row['type'];
$prosedur = str_replace('&', 'dan', $row['prosedur']);
...
$text2 = preg_replace('/[^A-Za-z0-9\  ]/', '', $birth_date);
$spc_needs = addslashes($row['spc_needs']);
$spc_needs = str_replace("&", "dan", $spc_needs);
$payment = $row['payment'];
...
file_put_contents('risworklistrefresh.xml', $xml->asXML());



$query = "INSERT INTO xray_exam
        (uid,acc,patientid,mrn,name,lastname, address, sex,birth_date,weight,name_dep,xray_type_code,typename,type,prosedur,dokterid,named,lastnamed,email,radiographer_id,radiographer_name,radiographer_lastname,create_time,schedule_date,schedule_time,contrast,priority,pat_state,contrast_allergies,spc_needs,payment,arrive_date,arrive_time,dokradid,dokrad_name,dokrad_lastname,fromorder)
        VALUES
        ('$uid', '$acc', '$patientid', '$mrn', '$name', '$lastname', '$address', '$sex', '$birth_date', '$weight', '$name_dep', '$xray_type_code', '$typename', '$type', '$prosedur','$dokterid', '$named', '$lastnamed','$email','$radiographer_id','$radiographer_name','$radiographer_lastname','$create_time', '$schedule_date', '$schedule_time', '$contrast', '$priority', '$pat_state', '$contrast_allergies', '$spc_needs','$payment', NOW(), NOW(), '$dokradid', '$dokrad_name', '$dokrad_lastname','$fromorder')";

if (!mysqli_query($conn, $query)) {
  echo ("Error description: " . mysqli_error($conn));
}

mysqli_query($conn, "UPDATE xray_order SET fromorder = 'simrssend', deleted_at = NOW() WHERE uid = '$uid'");
...
header("location:sendtojavarefresh.php");
