...
try {
    ...
    $client = new Client([
        'base_uri' => 'http://192.168.68.62:8000',
    ]);

    // get
    $responseGet = $client->request('GET', '/api/orders?uid=' . $uid);
    ...
    $workload = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM xray_workload_radiographer WHERE uid = '$uid'"));
    ...
    if ($workload['mrn'] == $mrn) {
        ...
        // try {
        $responsePost = $client->request('POST', '/api/orders/', [
            'json' => [
                "uid" => $uid,
            ],
            'http_errors' => false
