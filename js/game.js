// <script>
        ...
        type: 'bar',
        data: {
        labels: ["Dr. Acil Nomi", "Dr. Hardian Kristi", "Dr. Bella Salampessy"],
        datasets: [{
        label: '',
        data: [
        <?php
        $jumlah_acil = mysqli_query($conn,"select * from xray_exam where named='Dr. Acil'");
        echo mysqli_num_rows($jumlah_acil);
        ?>,
        <?php
        $jumlah_hardian = mysqli_query($conn,"select * from xray_exam where named='Dr. Hardian'");
        echo mysqli_num_rows($jumlah_hardian);
        ?>,
        <?php
        $jumlah_bella = mysqli_query($conn,"select * from xray_exam where named='Dr. Bella'");
        echo mysqli_num_rows($jumlah_bella);
        ?>
        ],
        backgroundColor: [
        'rgba(255, 99, 132, 0.2)',
