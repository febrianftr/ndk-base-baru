...
$("#post-acc-number").validate({
  ...
  submitHandler: function (form) {
    ...
    $.ajax({
      ...
      url: `http://${
        location.hostname
      }:20007/api/validation-workload-accession-no/${accession_no.val()}/${pat_id.val()}?mods_in_study=${mods_in_study}`,
      data: {
            ...
            .then((result) => {
              if (result) {
                $.ajax({
                  ...
                  url: `http://${
                    location.hostname
                  }:20007/api/update-workload-accession-no/${accession_no.val()}/${pat_id.val()}?mods_in_study=${mods_in_study}`,
                  data: {
                    study_iuid: study_iuid,
                    accession_no: accession_no.val(),
                    pat_id: pat_id.val(),
...
$("#edit-workload").validate({
  ...
  submitHandler: function (form) {
    ...
    $.ajax({
      type: "POST",
      url: `http://${location.hostname}:20007/api/update-workload/${study_iuid}`,
      data: $(form).serialize(),
      beforeSend: function () {
        $(".loading").show();
        $(".ubah").hide();
